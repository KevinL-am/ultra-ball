...
# 1. 頁面設定
st.set_page_config(page_title="TCG Ultra Quant (Stealth Mode)", page_icon="🟡", layout="wide")
...
st.markdown("""
    ...
    .price-box {
        font-size: 45px; font-weight: 900; color: #d4a017;
        text-align: center; padding: 25px; border: 5px solid #ffcc00;
...
# 5. 「最強隱形」捕捉引擎
def fetch_stealth_data(url):
    with sync_playwright() as p:
        # 使用 Chromium 並加入防偵測參數
        ...
        try:
            ...
            time.sleep(random.uniform(3, 5)) # 隨機等 3-5 秒，扮睇緊網頁
            
            # 2. 搵 PSA 10 粒掣
            # 由於有「圖靈」保護，我哋用 JavaScript 嚟撳掣，避開實體點擊偵測
            psa10_btn = page.locator('button:has-text("PSA 10")').first
            if psa10_btn.is_visible():
                psa10_btn.evaluate("btn => btn.click()")
                time.sleep(random.uniform(2, 4)) # 等價錢跳
            
            # 3. 執個「HK $」價錢
            # 我哋直接攞網頁最後渲染出嚟嘅文字
            content = page.content()
            price_match = re.search(r'HK\s?\$[\d,]+', content)
            price_text = price_match.group(0) if price_match else "N/A"
            ...
            return {"名稱": "隱形失敗", "價格": "被攔截", "網址": url}
        finally:
            browser.close()

# 6. UI
st.title("🟡 隱形高級球 (Anti-Turing Edition)")
st.info("已啟動 Stealth 模式，避開防爬偵測。")
run = st.button("H")
...
if run:
    if main_sheet:
        ...
        if urls:
            status = st.status("🟡 隱形球運作中，請耐心等候真人模擬...")
            results = []
            for url in urls:
                res = fetch_stealth_data(url)
                if res:
                    results.append(res)
            ...
            status.update(label="✅ 突破圖靈防線，捕捉成功！", state="complete")
